...
사전 학습 데이터의 경우 코드, 블로그, 기사, 광고 등 다양한 글이 섞여 있고 이 동안 LLM이 언어에 대한 전체적인 이해도가 높아지고 바로 다음에 올 단어를 점점 더 잘 예측하게 된다.

LLM이 사용자의 요청에 적절히 응답하기 위해서는 요청과 응답이 잘 연결되도록 추가로 학습하는데, 이를 지도 미세 조정(supervised fine-tuning)이라고 한다.

> 지도 미세 조정을 통해 LLM은 사용자의 요청에 맞춰 응답하도록 학습하는데, 이를 정렬(alignment)이라고 한다.

지도 미세 조정에 사용하는 데이터셋을 지시 데이터셋(instruction dataset)ㅇ라고 부른다.

지시 데이터셋의 형태

- 지시사항(instruction): 사용자의 요구사항을 표현한 문장
- 입력(input): 답변을 하는 데 필요한 데이터
- 출력(output): 지시사항, 입력, 출력을 정해진 포맷으로 하나로 묶은 데이터

LLM은 지시 데이터셋을 통해 사전 학습 때와 동일하게 다음 단어를 예측하는 인과적 언어 모델링(causal language modeling)사용해 학습한다.

좋은 지시 데이터셋이 갖춰야 할 조건

- 데이터 양은 1000개 정도 수준도 좋은 성능을 낼 수 있음
- 지시 데이터셋에서 지시사항이 다양한 형태로 되어 있고 응답 데이터의 품질이 높을수록 정렬한 모델의 답변 품질이 높아짐
  - 피상적인 정렬 가설(superficial alignment hypothesis): 모델의 지식이나 능력은 사전 학습 단계에서 대부분 학습하고 정렬 데이터를 통해서는 답변의 형식이나 모델이 능력과 지식을 어떻게 나열할지 정도만 추가로 배우기 때문에 적은 정렬 데이터로도 사용자가 원하는 형태의 답변을 생성할 수 있다는 가설
  - 데이터셋의 품질이 높을 경우 모델의 크기가 작아도 좋은 성능을 낼 수 있음
- 메타와 MS의 연구를 통한 좋은 지시 데이터셋 조건
...
지도 미세 조정을 마친 LLM에 지시사항을 입력해 여러 응답을 생성하여 선호 데이터셋을 구축하여 선호 데이터에 비선호 데이터보다 높은 점수를 주도록 채점 모델을 학습시켜서 더 원하는 답변을 하도록 할 수 있다.

> 가독성 높은 코드를 작성하도록 하거나, 사용자에게 결과적으로 해가 될 수 있는 정보(예: 약물 제조 방법, 폭탄 제조 방법)나 차별적인 답변을 하지 않도록 할 수 있다.

사람의 피드백을 활용한 강화 학습(Reinforcement Learning from Human Feedback, RLHF)

- 강화 학습에서는 `에이전트(agent)`가 `환경(environment)`에서 `행동(action)`을 한다. 행동에 따라 환경의 `상태(state)` 가 바뀌고 행동에 대한 보상(reward)이 생기는데, 에이전트는 이 변화된 상태를 인식하고 보상을 받는다. 에이전트는 가능하면 더 많은 보상을 받을 수 있도록 행동을 수정하면서 학습하는데, 이때 에이전트가 연속적으로 수행하는 행동의 모음을 `에피소드(episode)`라고 한다.
- 이떄 보상을 높게 받는 데에만 집중하는 보상 해킹(reward hacking)이 발생할 수 있다
  - 예를 들어, 코드 가독성 점수를 높게 받는 방법으로 깔끔한 코드를 작성하는 게 아니라 아예 코드를 작성하지 않거나 print("Hello World) 와 같은 간단한 코드만 작성해서 코드 가독성 점수를 높게 받으려고 할 수 있다.
  - OpenAI는 이를 피하기 위해 PPO라는 강화 학습 방법을 사용

근접 정책 최적화(Proximal Preference Optimization, PPO)

- 너무 멀지 않게 가까운 범위에서 리워드 모델의 높은 점수를 찾도록 한다는 의미
- 지도 미세 조정 모델을 기준으로 거리를 측정하기 때문에 참고 모델(reference model)이라고 함

RLHF 단점

- 리워드 모델을 학습시켜야 되는데, 리워드 모델의 성능이 좋지 않으면 LLM이 일관성 없는 점수를 통해 학습하게 되면서 제대로 학습되지 않는다.
  - 성능이 높고 일관성 있는(robust) 리워드 모델을 만들어야 한다.
- 모델을 학습시킬 때 참고 모델, 학습 모델, 리워드 모델총 3개의 모델이 필요하기 때문에 GPU와 같은 리소스가 더 많이 필요하다.
- 강화 학습 자체가 하이퍼파라미터에 민감하고 불안정

강화 학습 없이 LLM 학습하는 법

- 여러 생성 결과 중 리워드 모델이 가장 높은 점수를 준 결과를 LLM의 지도 미세 조정에 사용하는 `기각 샘플링(rejecting sampling)`
- 선호 데이터셋을 직접 LLM이 학습하는 방식으로 변경해 열풍을 일으킨 `직접 선호 최적화(Direct Preference Optimization, DPO)`

## Chapter 05. GPU 효율적인 학습

GPU를 1개만 사용할 때도 GPU의 메모리를 효율적으로 사용할 수 있는 **그레디언트 누적**, **그레디언트 체크포인팅**

그레디언트 누적

- 배치 크기를 키우면 더 빠르게 수렴하고 성능이 높아지지만 순전파 상태 저장에 필요한 메모리가 증가하면서 OOM 발생 가능
- 제한된 메모리 안에서 배치 크기를 키우는 것과 동일한 효과
- training_args 에서 gradient_accumulation_steps 설정
  - 손실을 gradient_accumulation_steps로 나눠서 역전파를 수행하고, gradient_accumulation_steps 번의 스탭마다 모델을 업데이트
  - 배치 크키 * 그레디언트 누적 스텝 수 = 총 배치 크기
- 추가적인 순전파 및 역전파 연산을 수행하기 때문에 학습시간 증가

그레디언트 체크포인팅

- 딥러닝 모델에서는 모델 업데이트를 위한 그레디언트를 계산하기 위해 순전파와 역전파를 수행하는데, 이때 역전파 계산을 위해 순전파의 결과를 저장하고 있어야 함.
  - 기본적인 저장 방식은 모두 저장하는 것인데 이렇게 되면 GPU 메모리를 많이 차지하게 되므로 역전파를 진행하면서 메모리를 절약하기 위해 사용이 끝난 데이터는 삭제
  - 다음으로는 역전파를 계산할 때 필요한 최소 데이터만 저장하고 나머지는 필요할 때 다시 계산하는 방식
  - 메모리를 효율적으로 쓸 수 있다는 **장점**이 있지만 한 번의 역전파를 위해 순전파를 반복적으로 계산해야 한다는 **단점**
- 위 방식을 절충하기 위한 방법이 그레디언트 체크포인팅
  - 순전파의 전체를 저장하거나 마지막만 저장하는 게 아니라, 중간중간에 값들을 저장(체크포인트)해서 메모리 사용을 줄이고 필요한 경우 체크포인트부터 다시 계산해 순전파 계산량도 줄임

분산 학습은 GPU 를 여러 개 활용해 딥러닝 모델을 학습시키는 것

- 모델의 학습 속도 증가: 데이터 병렬화
- 1개의 GPU로 학습이 어려운 모델 다룰 수 있음: 모델 명렬화
  - 딥러닝 모델의 층별로 나눠 GPU에 올리는 파이프라인 병렬화
  - 한 층의 모델도 나눠서 GPU에 올리는 텐셔 병렬화

LLM과 같은 기반 모델의 크기가 커지면서 하나의 GPU를 사용해 full find-tuning 을 수행하기 어려워졌다. 하지만 대부분의 개인과 조직은 여러 GPU를 사용해 모델을 학습시키기 어렵기 때문에 일부 파라미터만 학습하는 PEFT(Parameter Efficient Fine-Tuning) 방법  연구가 활발히 이뤄지고 있다. 그중에서도 LLM 학습에서 가장 주목받고 많이 활용되는 학습 방법은 모델에 일부 파라미터를 추가하고 그 부분만 학습하는 LoRA(Low-Rank Adaptation)학습 방식이다.

LoRA

- 모델 파라미터를 재구성해 더 적은 파라미터를 학습함으로써 GPU 메모리 사용량을 줄임
- 파라미터 재구성은 행렬을 더 작은 2개의 행렬 곱으로 표현해 전체 파라미터를 수정하는 것이 아니라 더 작은 2개의 행렬을 수정하는 것을 의미

LoRA 를 적용해야 할 때 결정해야 할 사항

- 차원 r
  - r을 작게 설정하면 학습시켜야 하는 파라미터의 수가 줄어들기 때문에 GPU 메모리 사용량을 줄일 수 있음
  - 하지만 r이 작아질 경우 그만큼 모델이 학습할 수 있는 용량이 작아지기 때문에 학습 데이터의 패턴을 충분히 학습하지 못할 수 있음
  - 실험을 통해 적절한 값을 설정
- 알파
  - 추가한 파라미터를 기존 파라미터에 얼마나 많이 반영할지
  - 추론할 때 추가한 행렬 A, B를 곱한 부분을 기존 파라미터 W와 동일하게 더해줄 수도 있지만, 행렬 A와 B를 더 중요하게 고려할 수도 있음
  - 알파/r 만큼의 비중으로 기존 파라미터 W에 더해줌
  - 실험을 통해 r값과 함께 적절히 설정
- 모델에 있는 많은 파라미터 중에서 어떤 파라미터를 재구성할지 결정
  - 일반적으로 셀프 어텐션 연산의 쿼리, 키, 값 가중치와 피드 포워드 층의 가중치와 같이 선형 연산의 가중치를 재구성
  - 잊 중에서 특정 가중치에만 LoRA를 적용할 수도 있고 전체 선형 층에 LoRA를 적용할 수도 있다
  - 보통 전체 선형 층에 LoRA를 적용한 경우 성능이 가장 좋다고 알려져 있는데, 이 부분 또한 실험을 통해 적절히 선택
