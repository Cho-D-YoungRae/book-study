...
- 상호 작용 스타일의 선택은 전체 애플리케이션의 가용성에 영향을 끼치며, 적합한 통합 테스트 전략을 수립하는 데에도 도움
- 클라이언트/서비스 상호 작용 스타일
  - 일대일/일대다 여부
...
- 클라이언트는 응답 대기 도중에 블로킹 되기 때문ㄴ에 서비스 실패는 클라이언트의 클라이언트로 거슬러 올라가면서 전체 시스템의 중단을 초래할 위험도 있음
...
- 무응답 원격 서비스를 어떻게 복구하면 좋을지는 그떄그떄 상황에 맞게 판단
...
- 필자는 가능한 한 플랫폼에서 제공되는 서비스 디스커버리를 사용할 것을 권장
...
- 클라이언트는 수행할 작업과 매개변수가 담긴 커맨드 메시지를 서비스가 소유한 점대점 메시징 채널에 보냄
- 서비스는 요청을 처리한 후 그 결과가 담긴 응답 메시지를 클라이언트가 소유한 점대점 채널로 돌려보냄
...
- 메시징 브로커는 여러 수신자 인스턴스를 묶어 마치 동일한 논리 수신자처럼 취급합니다(아파치 카프카 용어로 컨슈머 그룹). 멧지ㅣ 브로커는 각 샤드를 하나의 수신자에 배정하고, 수신자가 시동/종료하면 샤드를 재배정합니다.

### 3.3.6 중복 메시지 처리

- 메시지 브로커가 각 메시지를 꼭 한 번만 전달하면 좋겠지만, 그렇게 강제하려면 그만큼 값비싼 대가를 치러야 합니다. 그래서 메시지 브로커는 보통 적어도 한 번 이상 메시지를 전달하겠노라 약속합니다.
- 시스템이 정상일 때 각 메시지를 한 번만 전달
...
- RDBMS 기반의 애플리케이션이라면 DB 테이블을 임시 메시지 큐로 사용하는 `트랜잭셔널 아웃 박스 패턴`
...
- 메시지 브로커에도 클라이언트 라이브러리를 직접 사용했을 때의 문제
  - 메시지 브로커 API 에 메시지를 발행하는 비즈니스 로직이 클라이언트 라이브러리와 결합됩니다.
  - 멧지ㅣ 브로커의 클라이언트 라이브러리는 대부분 저수준이고 메시지를 주고받는 코드가 꽤 긴 편입니다.
...
- 요청 처리 도중 동기 통신을 제거하려면
  ...
  - 메시지를 OUTBOX 테이블에 삽입한느 식으로 DB 업데이트
  - 클라이언트에 응답을 반환
